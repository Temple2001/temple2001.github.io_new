---
title: 멋사 -  4주차 DB 세션
description: 백엔드의 오아시스라고 불리는 데이터베이스에 대해 알아보았습니다.
...
| --- | --- | --- | --- |
| 정형데이터
(Structured Data) | 미리 정의된 형식이 있는 데이터 | RDBMS
 | Entity-Relationship |
| 반정형데이터
(Semi-Structured Data) | 데이터의 구조 정보를 
데이터와 함께 제공하는 파일 형식의 데이터
→ 데이터의 형식과 구조가 변경될 수 있는 데이터 | NoSQL | Key-Value 등 |
| 비정형데이터
(Unstructured Data) | 미리 정의된 형식이 없는 데이터 | 비구조 | Non Structure |
...
- 다대다(N:M) 관계
    - 관계를 가진 **양쪽 엔티티 모두**에서 `1:N 관계`를 가지는 것을 의미한다.
...
| 필드타입 | 필수 옵션 | 설명 |
| --- | --- | --- |
| AutoField | - | auto increment 되는 IntegerField입니다. 
django에선 따로 명시하지 않을 시, id필드를 자동으로 추가해준다. |
...
| 명령어 | 내용 |
| --- | --- |
| python http://manage.py makemigrations	 | 앱 내의 migration 폴더를 만들어서 models.py의 변경사항 저장
*git add 명령어와 유사하다고 생각하면 좋을 것 같습니다!
**모델과 관련하여 수정이 발생한다면 해당 명령어를 먼저 진행해야 제대로 migrate 됩니다! |
| python http://manage.py migrate | Migration 폴더를 실행시켜 데이터베이스에 적용. |
...
![image.png](https://prod-files-secure.s3.us-west-2.amazonaws.com/ebe0ce4d-3d79-41f4-9c43-c22ad1b6eafa/08f238a9-ba9c-41a2-b26b-e2b047e4543e/06286a46-c208-4eb0-bc97-255b194fd4ed.png)
