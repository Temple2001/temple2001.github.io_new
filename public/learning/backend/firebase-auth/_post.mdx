...
토큰 생성은 Jwts 라이브러리를 이용해 생성한다. (자세한 코드는 생략)
...
검증이 통과되면 `SecurityContextHolder.getContext().setAuthentication(auth)`를 통해 인증된 토큰의 정보를 현재 유저의 인증 정보로 설정한다. 이를 통해 요청을 보낸 유저가 어떤 권한을 가지고 있는지 파악할 수 있는 **인가** 과정을 수행할 수 있게 된다.